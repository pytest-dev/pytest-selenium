...
def session_capabilities(pytestconfig):
    ...
    options = OPTIONS[browser.lower()]()

    for name, value in capabilities.items():
        options.set_capability(name, value)

    pytestconfig._options = options
    return options
...
# def session_capabilities(pytestconfig):
#     """Returns combined capabilities from pytest-variables and command line"""
#     capabilities = pytestconfig._capabilities
#     driver = pytestconfig.getoption("driver")
#     if driver.upper() == "REMOTE":
#
#     module = importlib.import_module(SUPPORTED_DRIVERS[driver], "selenium.webdriver")
#     try:
#         options = module.options.Options()
#     except AttributeError:
#         # Remote driver doesn't have options
#         browser = capabilities.get("browserName", "")
#         module = importlib.import_module(f".{browser}", "selenium.webdriver")
#         options = module.options.Options()
#
#     for name, value in capabilities.items():
#         options.set_capability(name, value)
...
def options(pytestconfig):
    return pytestconfig._options


@pytest.fixture
def service(driver_class, capabilities):
    if driver_class != webdriver.Remote:
        service = SERVICE[capabilities["browserName"]]
        service = service(
            driver_path,
        )
...
def capabilities(
    ...
    #     if all([key, options]):
    #         capabilities[key] =
    #         _merge(capabilities.get(key, {}), options.get(key, {}))
    for name, value in get_capabilities_from_markers(request.node).items():
        session_capabilities.set_capability(name, value)
